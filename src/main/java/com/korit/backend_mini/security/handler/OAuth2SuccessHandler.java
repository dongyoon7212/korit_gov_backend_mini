...
public class OAuth2SuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if (foundOAuth2User.isEmpty()) {
            response.sendRedirect("http://usedprice.store/auth/oauth2?provider="+provider+"&providerUserId="+providerUserId+"&email="+email);
            return;
        }
        ...
        response.sendRedirect("http://usedprice.store/auth/oauth2/signin?accessToken="+accessToken);
    }
}
